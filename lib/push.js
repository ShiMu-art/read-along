...
const OUTBOX_LOG = path.join(require("./store").DATA_DIR, "outbox.log");
...
const PUSH_ENABLED = Boolean(EFFECTIVE_WEBHOOK) || process.env.READING_PUSH_ENABLED === "1";

// 如果 webhook 没设置，默认指向自己的收件箱
const SELF_PORT = process.env.PORT || process.env.READING_PORT || "18004";
const EFFECTIVE_WEBHOOK = WEBHOOK || `http://localhost:${SELF_PORT}/api/push-inbox`;
...
function enqueueSystemMessage(text) {
  ...
  try {
    if (EFFECTIVE_WEBHOOK) return pushWebhook(trimmed);
